"""Spawn-safe synthetic training worker.

The worker owns no project-service objects and never opens SQLite.  It accepts
the value-only :mod:`training_protocol` request, reports the same versioned
messages over a multiprocessing queue, and leaves the service a checksum
manifest in the requested staging directory when training completes.
"""
...
def run_worker(
    ...
    *,
    ...
    try:
        ...
        inputs, targets = _synthetic_batch(config, device)
        ...
        if _is_cancelled(cancel_event):
            _emit_cancelled(output_queue, request_value, config, log_lines)
            return
        _write_staged_artifacts(staging, model, config, device, loss_value, total_steps)
        terminal_message = (
