...
ARTIFACT_PATH = REPO_ROOT / "docs/demo/ticket38-core-union-probe-contract.json"


class Ticket38CoreUnionProbeContractTest(unittest.TestCase):
    def test_builder_freezes_dense_core_union_probe(self) -> None:
        ...
        self.assertEqual(
            tuple(tuple(item) for item in membership["case_specs"]), CASE_SPECS
        )
        ...
        self.assertEqual(
            contract["evaluation"]["remote_response"],
            {"gating": "non_gating_diagnostic", "claim": "sparse_tolerance_not_segmentation_mask"},
        )
        ...
        self.assertTrue(
            all(item["bytes"] is None for item in contract["candidate_artifacts"]["roles"])
        )
        self.assertTrue(
            all(item["sha256"] is None for item in contract["candidate_artifacts"]["roles"])
        )
        self.assertEqual(
            contract["execution"],
            {
        ...
        self.assertEqual(
            canonical_ticket38_core_union_probe_contract_json(contract) + "\n",
            ARTIFACT_PATH.read_text(encoding="utf-8"),
        )

    def test_rejects_objective_membership_gate_path_and_source_drift(self) -> None:
        base = build_ticket38_core_union_probe_contract()
        mutations = (
            (
                lambda value: value["objective"]["terms"].append("per_instance_coverage_loss"),
                "objective",
            ),
            (
            ...
            (
                lambda value: value["membership"]["case_ids"].reverse(),
                "membership",
            ),
            (
                lambda value: value["candidate_artifacts"]["roles"][0].update(
                    path="artifacts/ticket38-core-union-probe/../escape.pt"
                ),
                "path",
            ),
            (
            ...
            (
                lambda value: value["candidate_artifacts"]["roles"][0].update(bytes=1),
                "hashes",
            ),
        )
        for mutate, message in mutations:
            ...
            with self.subTest(message=message), self.assertRaisesRegex(ValueError, message):
                validate_ticket38_core_union_probe_contract(tampered)
