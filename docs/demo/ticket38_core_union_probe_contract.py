"""Declarative dense core-union probe contract for Ticket 38."""
...
SCHEMA = "ticket38-core-union-probe-contract.v1"
SOURCE_WIDTH = 1536
SOURCE_HEIGHT = 1536
CLASS_ORDER = ("scratch", "particle")
CASE_SPECS = (("scratch", 150, 0), ("particle", 0, 150))
CASE_IDS = (
...
CANDIDATE_ARTIFACT_ROOT = "artifacts/ticket38-core-union-probe"
ARTIFACT_ROLES = (
    "checkpoint",
    "confidence_maps",
    "proposal_matching",
...
def build_ticket38_core_union_probe_contract() -> dict[str, object]:
    """Build the dense probe contract without pixels or model execution."""
...
def validate_ticket38_core_union_probe_contract(
    contract: Mapping[str, object],
    *,
    repo_root: Path | None = None,
    source_texts: Mapping[str, str] | None = None,
) -> None:
    """Fail closed on contract drift, source drift, and unsafe artifact paths."""

    if not isinstance(contract, Mapping):
        raise ValueError("Ticket 38 core-union probe contract must be an object")
    expected = build_ticket38_core_union_probe_contract()
    if set(contract) != set(expected):
        raise ValueError("Ticket 38 core-union probe contract fields drift")
    for key, value in expected.items():
        if key not in {"source_dependencies", "candidate_artifacts"} and contract.get(key) != value:
            raise ValueError(f"Ticket 38 {key} contract drift")
...
def canonical_ticket38_core_union_probe_contract_json(
    contract: Mapping[str, object],
) -> str:
    """Return canonical JSON after validating the declarative contract."""

    validate_ticket38_core_union_probe_contract(contract)
...
def canonical_contract_json(contract: Mapping[str, object]) -> str:
    return canonical_ticket38_core_union_probe_contract_json(contract)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if output.exists():
            raise ValueError(f"Ticket 38 core-union probe contract output already exists: {output}")
        output.parent.mkdir(parents=True, exist_ok=True)
        text = canonical_ticket38_core_union_probe_contract_json(
            build_ticket38_core_union_probe_contract()
        ) + "\n"
        with output.open("x", encoding="utf-8", newline="\n") as stream:
            stream.write(text)
    except (OSError, TypeError, ValueError) as error:
        print(f"Ticket 38 core-union probe contract failed: {error}", file=sys.stderr)
        return 1
    print(f"Ticket 38 core-union probe contract: {output}", flush=True)
    return 0
...
def _validate_source_dependencies(
    ...
    *,
    ...
    expected = build_ticket38_core_union_probe_contract()["source_dependencies"]
    if value != expected:
        raise ValueError("Ticket 38 source dependency contract drift")
    if source_texts is not None:
        if not isinstance(source_texts, Mapping):
            raise ValueError("Ticket 38 source texts must be an object")
        paths = {path for path, _sha256 in SOURCE_DEPENDENCIES}
        if any(path not in paths for path in source_texts):
            raise ValueError("Ticket 38 source text path is not declared")
    ...
    for path, expected_sha256 in SOURCE_DEPENDENCIES:
        ...
        if actual != expected_sha256:
            raise ValueError(f"Ticket 38 source dependency SHA-256 mismatch: {path}")


def _validate_candidate_artifacts(value: object) -> None:
    expected = build_ticket38_core_union_probe_contract()["candidate_artifacts"]
    if not isinstance(value, Mapping) or set(value) != set(expected):
        raise ValueError("Ticket 38 candidate artifact fields drift")
    if value.get("root") != CANDIDATE_ARTIFACT_ROOT:
        raise ValueError("Ticket 38 candidate artifact root drift")
    if value.get("hash_policy") != HASH_POLICY:
        raise ValueError("Ticket 38 candidate artifact hash policy drift")
    if value.get("seal_status") != "future_artifact_seal_required":
        raise ValueError("Ticket 38 candidate artifact seal status drift")
    roles = value.get("roles")
    if not isinstance(roles, list) or len(roles) != len(ARTIFACT_ROLES):
        raise ValueError("Ticket 38 candidate artifact roles drift")
    seen_roles: set[str] = set()
    seen_paths: set[str] = set()
    for item in roles:
        if not isinstance(item, Mapping) or set(item) != {"role", "path", "bytes", "sha256"}:
            raise ValueError("Ticket 38 candidate artifact role fields drift")
        role = item["role"]
        path = item["path"]
        if not isinstance(role, str) or role not in ARTIFACT_ROLES or role in seen_roles:
            raise ValueError("Ticket 38 candidate artifact roles must be unique and allowed")
        seen_roles.add(role)
        _validate_artifact_path(path)
        if path in seen_paths:
            raise ValueError("Ticket 38 candidate artifact paths must be unique")
        seen_paths.add(path)
        if path != f"{CANDIDATE_ARTIFACT_ROOT}/{ARTIFACT_FILENAMES[role]}":
            raise ValueError("Ticket 38 candidate artifact path does not match role")
        if item["bytes"] is not None or item["sha256"] is not None:
            raise ValueError("Ticket 38 candidate artifact hashes must remain pending")
    if tuple(item["role"] for item in roles) != ARTIFACT_ROLES:
        raise ValueError("Ticket 38 candidate artifact role order drift")


def _validate_artifact_path(path: object) -> None:
    if not isinstance(path, str) or not path:
        raise ValueError("Ticket 38 candidate artifact path must be relative")
    lowered = path.lower()
    if any(token in lowered for token in _FORBIDDEN_PATH_TOKENS):
        raise ValueError("Ticket 38 candidate artifact path contains forbidden evidence token")
    if (
        path.startswith(("/", "\\"))
        ...
        raise ValueError("Ticket 38 candidate artifact path must reject absolute or traversal paths")
    if not path.startswith(CANDIDATE_ARTIFACT_ROOT + "/"):
        raise ValueError("Ticket 38 candidate artifact path is outside allowed root")
...
def _read_normalized_file_bytes(path: Path, label: str) -> bytes:
    try:
        ...
        raise ValueError(f"cannot read Ticket 38 source dependency {label}: {error}") from error
