"""Ticket 38 core-union gate runner and sealed-map evaluator."""
...
SCHEMA = "ticket38-core-union-gate.v1"
CHECKPOINT_SCHEMA = "ticket38-core-union-gate.checkpoint.v1"
MATCHING_SCHEMA = "ticket38-core-union-gate.matching.v1"
CONFIG_SCHEMA = "ticket38-core-union-gate.configuration.v1"
REPORT_PATH = Path("docs/demo/ticket38-core-union-gate.json")
IMAGE_WIDTH = 1536
IMAGE_HEIGHT = 1536
PATCH_SIZE = 128
PATCH_STRIDE = 64
...
def compute_ticket38_objective(
    logits: Tensor,
    targets: Tensor,
    ...
    *,
    ...
    """Return the frozen dense core-union objective with context terms."""
    ...
    local_full_far = far_negative_suppression_loss(
        logits,
    ...
    return core + far + local_full_far + CONTEXT_WEIGHTS["dense_absent_class_loss"] * dense + CONTEXT_WEIGHTS[
        "same_image_grid_ranking_loss"
    ] * ranking + CONTEXT_WEIGHTS["overlap_consistency_loss"] * overlap


def evaluate_ticket38_response(
    response: np.ndarray,
    ...
    *,
    ...
    row = {
        "truth_component_count": int(matching["truth_component_count"]),
...
def validate_ticket38_confidence_map(value: object) -> np.ndarray:
    """Require model scoring output to already be float32 probability maps."""

    if not isinstance(value, np.ndarray):
        raise ValueError("Ticket 38 confidence map must be a numpy array")
    if (
        ...
        raise ValueError("Ticket 38 confidence map must be finite float32 1536x1536x2 in [0,1]")
    return value


def evaluate_ticket38_stored_maps(
    ...
    if not isinstance(maps, Mapping) or tuple(maps) != CASE_IDS:
        raise ValueError("confidence map keys/order must match Ticket 38 case order")
    selected = tuple(
        case
        for case in (build_ticket36_cases() if cases is None else cases)
        if case.case_id in CASE_IDS
    )
    if tuple(case.case_id for case in selected) != CASE_IDS:
        raise ValueError("Ticket 38 map cases must match frozen case order")
    ...
    for case in selected:
        ...
        if confidence_map.dtype != np.float32 or confidence_map.shape != (IMAGE_HEIGHT, IMAGE_WIDTH, len(CLASS_ORDER)):
            raise ValueError("Ticket 38 confidence maps must be finite float32 1536x1536x2 arrays")
        if (
            not np.isfinite(confidence_map).all()
            or np.any(confidence_map < 0.0)
            or np.any(confidence_map > 1.0)
        ):
            raise ValueError("Ticket 38 confidence maps must be finite probabilities")
        ...
        row, case_matching = evaluate_ticket38_response(
            response,
            rendered,
...
def seal_ticket38_artifacts(
    ...
    *,
    ...
    try:
        ...
        raise ValueError("Ticket 38 candidate root must be inside repo_root") from error
    if relative_root != CANDIDATE_ARTIFACT_ROOT:
        raise ValueError("Ticket 38 candidate root drift")
    roles: list[dict[str, object]] = []
    for role in ARTIFACT_ROLES:
        path = root / ARTIFACT_FILENAMES[role]
        if not path.is_file():
            raise ValueError(f"Ticket 38 artifact is missing: {role}")
...
def verify_ticket38_artifact_seal(
    ...
    *,
    ...
    base = Path(__file__).resolve().parents[2] if repo_root is None else Path(repo_root).expanduser().resolve()
    _verify_ticket38_artifact_bytes(seal, base)
...
def _verify_ticket38_artifact_bytes(seal: Mapping[str, object], base: Path) -> None:
...
def preflight_ticket38_core_union_gate(
    *,
    ...
    contract_path = root / "docs/demo/ticket38-core-union-gate-contract.json"
    try:
        tracked_text = contract_path.read_text(encoding="utf-8")
        contract = json.loads(tracked_text)
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as error:
        raise ValueError(f"Ticket 38 tracked contract is unreadable: {error}") from error
    if tracked_text != canonical_ticket38_core_union_gate_contract_json(contract) + "\n":
        raise ValueError("Ticket 38 tracked contract is not canonical")
    validate_ticket38_core_union_gate_contract(contract, repo_root=root)
    _validate_runtime_contract(contract)
    _validate_output_targets(root, report)
    head, status = _git_state(root)
    if status:
        raise RuntimeError("Ticket 38 preflight requires a clean committed git worktree")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("Ticket 38 runner requires CUDA")
    return {
        "contract": contract,
        "git_head": head,
        "repo_root": root,
...
def _validate_runtime_contract(contract: Mapping[str, object]) -> None:
    ...
    if membership != expected_membership or tuple(CASE_IDS) != RUNTIME_CASE_IDS or tuple(CASE_SPECS) != RUNTIME_CASE_SPECS:
        raise ValueError("Ticket 38 runtime case contract drift")
    cases = tuple(case for case in build_ticket36_cases() if case.case_id in CASE_IDS)
    if tuple(case.case_id for case in cases) != RUNTIME_CASE_IDS or any(
        case.seed != 101 or case.split != "train" for case in cases
    ):
        raise ValueError("Ticket 38 runtime case builder drift")
    expected_recipe = {
    ...
    if contract.get("recipe") != expected_recipe:
        raise ValueError("Ticket 38 runtime recipe contract drift")
    if contract.get("deterministic_policy") != DETERMINISTIC_POLICY:
        raise ValueError("Ticket 38 deterministic policy contract drift")
    expected_objective = build_ticket38_core_union_gate_contract()["objective"]
    if contract.get("objective") != expected_objective:
        raise ValueError("Ticket 38 runtime objective contract drift")
    expected_evaluation = build_ticket38_core_union_gate_contract()["evaluation"]
    if contract.get("evaluation") != expected_evaluation:
        raise ValueError("Ticket 38 runtime evaluation contract drift")
    if ResNet18_Weights.DEFAULT is not ResNet18_Weights.IMAGENET1K_V1:
        raise RuntimeError("Ticket 38 ImageNet default weight enum drift")


def run_ticket38_core_union_gate(
    *,
    repo_root: Path | None = None,
    report_path: Path | None = None,
) -> dict[str, object]:
    """Execute the fixed four-case CUDA gate exactly once."""

    preflight = preflight_ticket38_core_union_gate(repo_root=repo_root, report_path=report_path)
    ...
    for case in cases:
        ...
        model, state, history = _train_ticket38_case(case, source)
        states[case.case_id] = state
        histories[case.case_id] = history
        maps[case.case_id] = validate_ticket38_confidence_map(_score_source_model(model, source))
        del model
    ...
    with np.load(maps_path, allow_pickle=False) as loaded:
        if tuple(loaded.files) != CASE_IDS:
            raise ValueError("Ticket 38 confidence map keys drift after write")
        reread_maps = {case_id: np.asarray(loaded[case_id]) for case_id in CASE_IDS}
    rows, matching = evaluate_ticket38_stored_maps(reread_maps, cases)
    ...
    seal = seal_ticket38_artifacts(candidate_root, repo_root=root)
    ...
    canonical_ticket38_core_union_gate_json(report, repo_root=root)
    _write_json(report_file, report)
    return report


def validate_ticket38_core_union_gate_report(
    ...
    *,
    ...
    if not isinstance(report, Mapping) or report.get("schema") != SCHEMA:
        raise ValueError("Ticket 38 candidate report schema drift")
    required = {
    ...
    if set(report) != required:
        raise ValueError("Ticket 38 candidate report fields drift")
    ...
    if report.get("contract_sha256") != expected_contract_sha:
        raise ValueError("Ticket 38 candidate report contract SHA drift")
    if report.get("case_order") != list(CASE_IDS):
        raise ValueError("Ticket 38 candidate report case order drift")
    if report.get("recipe") != contract["recipe"] or report.get("objective") != contract["objective"]:
        raise ValueError("Ticket 38 candidate report recipe/objective drift")
    if report.get("deterministic_policy") != contract["deterministic_policy"]:
        raise ValueError("Ticket 38 candidate report deterministic policy drift")
    if report.get("evaluation") != contract["evaluation"] or report.get("boundary") != contract["boundary"] or report.get("claims") != contract["claims"]:
        raise ValueError("Ticket 38 candidate report evaluation drift")
    if report.get("metrics_source") != "sealed_float32_re_read":
        raise ValueError("Ticket 38 candidate report metrics source drift")
    execution = report.get("execution")
    if not isinstance(execution, Mapping) or execution.get("gate_run_count") != 1 or execution.get("rerun") is not False or execution.get("final_epoch_only") is not True:
        raise ValueError("Ticket 38 candidate report execution drift")
    ...
    if not isinstance(boundary, Mapping) or boundary.get("cam_default") != "cam_v2" or boundary.get("spatial_mil_status") != "experimental":
        raise ValueError("Ticket 38 candidate report boundary drift")
    ...
    if not isinstance(rows, list) or tuple(row.get("case_id") for row in rows if isinstance(row, Mapping)) != CASE_IDS:
        raise ValueError("Ticket 38 candidate report rows drift")
    for row in rows:
        if not isinstance(row, Mapping):
            raise ValueError("Ticket 38 candidate report row must be an object")
        _validate_row(row)
    expected_overall = "PASS" if all(row["overall"] == "PASS" for row in rows) else "FAIL"
    if report.get("overall") != expected_overall:
        raise ValueError("Ticket 38 candidate report overall drift")
    ...
    if report.get("recommendation") != expected_recommendation:
        raise ValueError("Ticket 38 candidate report recommendation drift")
    base = Path(__file__).resolve().parents[2] if repo_root is None else Path(repo_root).expanduser().resolve()
    seal = report.get("artifact_seal")
    verify_ticket38_artifact_seal(seal, repo_root=base)
    ...
    expected_rows, expected_matching = evaluate_ticket38_stored_maps(maps)
    if list(rows) != expected_rows or matching != expected_matching:
        raise ValueError("Ticket 38 report/sealed maps or matching drift")
    ...
    if configuration.get("contract_sha256") != expected_contract_sha or configuration.get("recipe") != contract["recipe"] or configuration.get("objective") != contract["objective"] or configuration.get("metrics_source") != "sealed_float32_re_read":
        raise ValueError("Ticket 38 configuration contract drift")


def canonical_ticket38_core_union_gate_json(
    report: Mapping[str, object], *, repo_root: Path | None = None
) -> str:
    validate_ticket38_core_union_gate_report(report, repo_root=repo_root)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        report = run_ticket38_core_union_gate(report_path=args.report)
    except (OSError, RuntimeError, TypeError, ValueError) as error:
        print(f"Ticket 38 core-union gate failed: {error}", file=sys.stderr)
        return 1
    print(f"Ticket 38 core-union gate: {report['overall']}", flush=True)
    return 0


def _train_ticket38_case(case: DevelopmentCase, source: np.ndarray) -> tuple[torch.nn.Module, dict[str, Tensor], list[float]]:
    ...
    if not asserted:
        raise ValueError(f"Ticket 38 case has no asserted grid: {case.case_id}")
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"Ticket 38 {case.case_id}: epoch {epoch + 1}/{EPOCHS} loss={epoch_loss:.6f}", flush=True)
    if _snapshot_batch_norm(model) != initial_bn:
        raise RuntimeError("Ticket 38 BatchNorm running statistics drifted")
...
def _build_report(
    ...
    *,
    ...
    return {
        ...
        "execution": {"gate_run_count": 1, "rerun": False, "final_epoch_only": True},
        "metrics_source": "sealed_float32_re_read",
        "recipe": copy.deepcopy(contract["recipe"]),
        "objective": copy.deepcopy(contract["objective"]),
        "deterministic_policy": copy.deepcopy(dict(deterministic_policy)),
...
def _expected_case_counts(case_id: object) -> tuple[int, int, int] | None:
    if case_id not in CASE_IDS:
        return None
    for item in build_ticket38_core_union_gate_contract()["evaluation"]["cases"]:
        if item["case_id"] == case_id:
            return (int(item["truth_components"]), int(item["proposal_count"]), int(item["matched_count"]))
    raise ValueError(f"Ticket 38 case evaluation is missing: {case_id}")


def _row_passes(row: Mapping[str, object]) -> bool:
    expected_counts = _expected_case_counts(row.get("case_id"))
    counts_pass = expected_counts is None or (
        row.get("truth_component_count"),
        row.get("proposal_count"),
    ...
    return (
        ...
        and float(row["normal_grid_leak_rate"]) == 0.0
...
def _validate_row(row: Mapping[str, object]) -> None:
    required = {
        "case_id", "class_code", "truth_component_count", "proposal_count", "matched_count",
        "proposal_precision", "proposal_recall", "unique_truth_touch_recall",
    ...
    if set(row) != required or row.get("case_id") not in CASE_IDS or row.get("class_code") not in CLASS_ORDER:
        raise ValueError("Ticket 38 candidate report row fields drift")
    for field in ("truth_component_count", "proposal_count", "matched_count"):
        value = row[field]
        if isinstance(value, bool) or not isinstance(value, int) or value < 0:
            raise ValueError(f"Ticket 38 candidate report row {field} invalid")
    ...
    if isinstance(row["normal_grid_leaks"], bool) or not isinstance(row["normal_grid_leaks"], int) or row["normal_grid_leaks"] < 0:
        raise ValueError("Ticket 38 normal_grid_leaks invalid")
    for field in ("unmatched_truth_component_ids", "unmatched_proposal_ids", "cross_component_merge_proposal_ids"):
        if not isinstance(row[field], list) or any(isinstance(item, bool) or not isinstance(item, int) or item < 0 for item in row[field]):
            raise ValueError("Ticket 38 candidate report IDs invalid")
    if row["overall"] not in {"PASS", "FAIL"} or row["overall"] != ("PASS" if _row_passes(row) else "FAIL"):
        raise ValueError("Ticket 38 candidate report row gate drift")
...
def _validate_remote(remote: object) -> None:
    if not isinstance(remote, Mapping) or set(remote) != {"numerator", "denominator", "ratio"}:
        raise ValueError("Ticket 38 remote response diagnostic drift")
    ...
    if (
        ...
        raise ValueError("Ticket 38 remote response diagnostic invalid")
...
def _validate_seal_shape(seal: Mapping[str, object]) -> tuple[tuple[str, str, int, str], ...]:
    ...
    for index, item in enumerate(roles):
        ...
        if role != ARTIFACT_ROLES[index] or not isinstance(path, str) or path in seen_paths or path != f"{CANDIDATE_ARTIFACT_ROOT}/{ARTIFACT_FILENAMES[role]}":
            raise ValueError("Ticket 38 artifact seal role/path drift")
        seen_paths.add(path)
        if isinstance(item["bytes"], bool) or not isinstance(item["bytes"], int) or item["bytes"] <= 0:
            raise ValueError("Ticket 38 artifact seal bytes drift")
        if not isinstance(item["sha256"], str) or len(item["sha256"]) != 64 or item["sha256"] != item["sha256"].lower() or any(char not in "0123456789abcdef" for char in item["sha256"]):
            raise ValueError("Ticket 38 artifact seal sha256 drift")
...
def _validate_checkpoint_artifact(path: Path) -> None:
    try:
        ...
        raise ValueError("Ticket 38 checkpoint cannot be read safely") from error
    ...
    for state in state_dicts.values():
        ...
        if set(state) != set(expected_state):
            raise ValueError("Ticket 38 checkpoint state keys drift")
        for name, expected in expected_state.items():
            ...
            if not isinstance(value, Tensor) or value.device.type != "cpu" or value.shape != expected.shape or value.dtype != expected.dtype:
                raise ValueError(f"Ticket 38 checkpoint state tensor drift: {name}")
            if torch.is_floating_point(value) and not torch.isfinite(value).all():
                raise ValueError("Ticket 38 checkpoint state contains non-finite tensor")
        try:
            model.load_state_dict(state, strict=True)
        except (RuntimeError, TypeError) as error:
            raise ValueError("Ticket 38 checkpoint strict v5 load failed") from error
...
def _read_float32_maps_artifact(path: Path) -> dict[str, np.ndarray]:
    try:
        with np.load(path, allow_pickle=False) as loaded:
            if tuple(loaded.files) != CASE_IDS:
                raise ValueError("Ticket 38 sealed map keys drift")
            maps = {}
            for case_id in CASE_IDS:
                ...
                if (
                    ...
                    raise ValueError("Ticket 38 sealed maps dtype/shape/finite drift")
                maps[case_id] = np.asarray(value)
            return maps
    except (OSError, ValueError) as error:
        raise ValueError("Ticket 38 sealed confidence maps invalid") from error


def _validate_matching_artifact(path: Path) -> None:
    value = _read_json_artifact(path)
    if value.get("schema") != MATCHING_SCHEMA or value.get("case_order") != list(CASE_IDS) or not isinstance(value.get("per_case"), Mapping) or set(value["per_case"]) != set(CASE_IDS):
        raise ValueError("Ticket 38 matching artifact contract drift")
    for case_id in CASE_IDS:
        item = value["per_case"].get(case_id)
        if not isinstance(item, Mapping) or len(item) != 1:
            raise ValueError("Ticket 38 matching artifact row drift")
        class_code, matching = next(iter(item.items()))
        if class_code not in CLASS_ORDER or not isinstance(matching, Mapping):
            raise ValueError("Ticket 38 matching artifact class drift")
        expected_counts = _expected_case_counts(case_id)
        if expected_counts is None or (
            matching.get("truth_component_count"),
            matching.get("proposal_count"),
            matching.get("matched_count"),
        ) != expected_counts:
            raise ValueError("Ticket 38 matching artifact count drift")
...
def _validate_configuration_artifact(path: Path) -> None:
    ...
    if (
        ...
        raise ValueError("Ticket 38 configuration environment drift")
    if value.get("deterministic_policy") != DETERMINISTIC_POLICY:
        raise ValueError("Ticket 38 configuration deterministic policy drift")
    ...
    if not isinstance(value.get("runner_sha256"), str) or value["runner_sha256"] != runner_expected or value["runner_sha256"] != _sha256_normalized_file(Path(__file__)):
        raise ValueError("Ticket 38 runner SHA drift")
    history = value.get("loss_history")
    if not isinstance(history, Mapping) or set(history) != set(CASE_IDS):
        raise ValueError("Ticket 38 loss history case order drift")
    for case_id in CASE_IDS:
        ...
        if not isinstance(values, list) or len(values) != EPOCHS or any(
            ...
            raise ValueError(f"Ticket 38 loss history drift: {case_id}")
...
def _read_json_artifact(path: Path) -> dict[str, object]:
    try:
        ...
        raise ValueError(f"Ticket 38 JSON artifact invalid: {path.name}") from error
    if not isinstance(value, dict):
        raise ValueError(f"Ticket 38 JSON artifact must be an object: {path.name}")
    return value
...
def _write_json(path: Path, value: Mapping[str, object]) -> None:
    ...
    try:
        ...
        raise ValueError(f"Ticket 38 JSON artifact write failed: {path}") from error
...
def _git_state(root: Path) -> tuple[str, str]:
    try:
        ...
        raise RuntimeError(f"Ticket 38 git preflight failed: {error}") from error
    return head, status
...
def _resolve_fixed_report_path(root: Path, report_path: Path | None) -> Path:
    ...
    if candidate != expected:
        raise ValueError("Ticket 38 report path is fixed")
    return candidate


def _validate_output_targets(root: Path, report: Path) -> None:
    if (root / CANDIDATE_ARTIFACT_ROOT).exists():
        raise FileExistsError("Ticket 38 candidate artifact root already exists")
    if report.exists():
        raise FileExistsError("Ticket 38 candidate report already exists")


def _contract_sha256(contract: Mapping[str, object]) -> str:
    return hashlib.sha256((canonical_ticket38_core_union_gate_contract_json(contract) + "\n").encode("utf-8")).hexdigest()
...
def _sha256_file(path: Path) -> str:
    try:
        ...
        raise ValueError(f"Ticket 38 ImageNet weight file is unreadable: {path}") from error
...
def _validate_cached_imagenet_weights(recipe: Mapping[str, object]) -> dict[str, str]:
    ...
    if (
        ...
        raise ValueError("Ticket 38 ImageNet weight contract drift")
    path = Path(torch.hub.get_dir()) / "checkpoints" / Path(WEIGHTS_URL).name
    if not path.is_file():
        raise RuntimeError(f"Ticket 38 cached ImageNet weight file is missing: {path}")
    digest = _sha256_file(path)
    if digest != WEIGHTS_SHA256:
        raise RuntimeError("Ticket 38 cached ImageNet weight SHA-256 drift")
    return {
        "enum": WEIGHTS_ID,
        "url": WEIGHTS_URL,
...
def _establish_deterministic_policy(policy: Mapping[str, object]) -> dict[str, object]:
    ...
    if (
        ...
        raise RuntimeError("Ticket 38 deterministic policy could not be established")
...
__all__ = [
    ...
    "WEIGHTS_SHA256",
    "WEIGHTS_URL",
    "canonical_ticket38_core_union_gate_json",
    "compute_ticket38_objective",
    "evaluate_ticket38_response",
    "evaluate_ticket38_stored_maps",
    "main",
    "preflight_ticket38_core_union_gate",
    "run_ticket38_core_union_gate",
    "seal_ticket38_artifacts",
    "validate_ticket38_core_union_gate_report",
    "validate_ticket38_checkpoint_artifact",
    "validate_ticket38_confidence_map",
    "validate_ticket38_maps_artifact",
    "verify_ticket38_artifact_seal",
]
