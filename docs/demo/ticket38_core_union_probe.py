"""Ticket 38 core-union probe runner and sealed-map evaluator."""
...
from docs.demo.ticket38_core_union_probe_contract import (
    ...
    canonical_ticket38_core_union_probe_contract_json,
    validate_ticket38_core_union_probe_contract,
)
...
SCHEMA = "ticket38-core-union-probe.v1"
CHECKPOINT_SCHEMA = "ticket38-core-union-probe.checkpoint.v1"
MATCHING_SCHEMA = "ticket38-core-union-probe.matching.v1"
CONFIG_SCHEMA = "ticket38-core-union-probe.configuration.v1"
REPORT_PATH = Path("docs/demo/ticket38-core-union-probe.json")
IMAGE_WIDTH = 1536
IMAGE_HEIGHT = 1536
PATCH_SIZE = 128
PATCH_STRIDE = 64
...
def preflight_ticket38_core_union_probe(
    *,
    repo_root: Path | None = None,
    ...
    contract_path = root / "docs/demo/ticket38-core-union-probe-contract.json"
    try:
    ...
    if tracked_text != canonical_ticket38_core_union_probe_contract_json(contract) + "\n":
        raise ValueError("Ticket 38 tracked contract is not canonical")
    validate_ticket38_core_union_probe_contract(contract, repo_root=root)
...
def _validate_runtime_contract(contract: Mapping[str, object]) -> None:
    ...
    expected_objective = build_ticket38_core_union_probe_contract()["objective"]
    if contract.get("objective") != expected_objective:
        raise ValueError("Ticket 38 runtime objective contract drift")
    expected_evaluation = build_ticket38_core_union_probe_contract()["evaluation"]
...
def run_ticket38_core_union_probe(
    *,
    repo_root: Path | None = None,
    report_path: Path | None = None,
) -> dict[str, object]:
    """Execute the fixed two-case CUDA probe exactly once."""

    preflight = preflight_ticket38_core_union_probe(repo_root=repo_root, report_path=report_path)
    ...
    canonical_ticket38_core_union_probe_json(report, repo_root=root)
    _write_json(report_file, report)
    return report


def validate_ticket38_core_union_probe_report(
    ...
    *,
    ...
    if not isinstance(execution, Mapping) or execution.get("probe_run_count") != 1 or execution.get("rerun") is not False or execution.get("final_epoch_only") is not True:
...
def canonical_ticket38_core_union_probe_json(
    report: Mapping[str, object], *, repo_root: Path | None = None
) -> str:
    validate_ticket38_core_union_probe_report(report, repo_root=repo_root)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        report = run_ticket38_core_union_probe(report_path=args.report)
    except (OSError, RuntimeError, TypeError, ValueError) as error:
        print(f"Ticket 38 core-union probe failed: {error}", file=sys.stderr)
        return 1
    print(f"Ticket 38 core-union probe: {report['overall']}", flush=True)
    return 0
...
def _build_report(
    ...
    *,
    ...
    return {
        ...
        "execution": {"probe_run_count": 1, "rerun": False, "final_epoch_only": True},
        "metrics_source": "sealed_float32_re_read",
        "recipe": copy.deepcopy(contract["recipe"]),
        "objective": copy.deepcopy(contract["objective"]),
        "deterministic_policy": copy.deepcopy(dict(deterministic_policy)),
...
def _validate_configuration_artifact(path: Path) -> None:
    ...
    runner_expected = next(
        ...
        if path == "docs/demo/ticket38_core_union_probe.py"
    )
...
def _contract_sha256(contract: Mapping[str, object]) -> str:
    return hashlib.sha256((canonical_ticket38_core_union_probe_contract_json(contract) + "\n").encode("utf-8")).hexdigest()
...
__all__ = [
    ...
    "WEIGHTS_SHA256",
    "WEIGHTS_URL",
    "canonical_ticket38_core_union_probe_json",
    "compute_ticket38_objective",
    "evaluate_ticket38_response",
    "evaluate_ticket38_stored_maps",
    "main",
    "preflight_ticket38_core_union_probe",
    "run_ticket38_core_union_probe",
    "seal_ticket38_artifacts",
    "validate_ticket38_core_union_probe_report",
