"""Declarative dense core-union gate contract for Ticket 38."""
...
SCHEMA = "ticket38-core-union-gate-contract.v1"
SOURCE_WIDTH = 1536
SOURCE_HEIGHT = 1536
...
CANDIDATE_ARTIFACT_ROOT = "artifacts/ticket38-core-union-gate"
ARTIFACT_ROLES = (
    "checkpoint",
    "confidence_maps",
    "proposal_matching",
...
SOURCE_DEPENDENCIES = (
    ...
    (
        ...
        "0f3a76c1e5cb88baf6fd5965a8273992212f76c3ef6e08746c873557db25c082",
    ),
    (
    ...
    (
        "docs/demo/ticket38_core_union_gate.py",
        "5690e1d426b30a6fff3fe85fbe90b15f595171cd4318c6022e300322844f9543",
    ),
    (
    ...
    (
        "docs/demo/ticket38-core-union-probe.json",
        "f8317d8b29623f4b2d4a130d8a200930503e9acc7a59a29fc0dca426a81bcc33",
    ),
)
...
def build_ticket38_core_union_gate_contract() -> dict[str, object]:
    """Build the dense four-case gate contract without execution."""
...
def validate_ticket38_core_union_gate_contract(
    contract: Mapping[str, object],
    *,
    repo_root: Path | None = None,
    source_texts: Mapping[str, str] | None = None,
) -> None:
    """Fail closed on gate drift, source drift, and unsafe artifact paths."""

    if not isinstance(contract, Mapping):
        raise ValueError("Ticket 38 core-union gate contract must be an object")
    expected = build_ticket38_core_union_gate_contract()
    if set(contract) != set(expected):
        raise ValueError("Ticket 38 core-union gate contract fields drift")
    for key, value in expected.items():
        if key not in {"source_dependencies", "candidate_artifacts"} and contract.get(key) != value:
            raise ValueError(f"Ticket 38 gate {key} contract drift")
...
def canonical_ticket38_core_union_gate_contract_json(
    contract: Mapping[str, object],
) -> str:
...
def canonical_contract_json(contract: Mapping[str, object]) -> str:
    return canonical_ticket38_core_union_gate_contract_json(contract)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if output.exists():
            raise ValueError(f"Ticket 38 core-union gate contract output already exists: {output}")
        output.parent.mkdir(parents=True, exist_ok=True)
        text = canonical_ticket38_core_union_gate_contract_json(
            build_ticket38_core_union_gate_contract()
        ) + "\n"
        with output.open("x", encoding="utf-8", newline="\n") as stream:
            stream.write(text)
    except (OSError, TypeError, ValueError) as error:
        print(f"Ticket 38 core-union gate contract failed: {error}", file=sys.stderr)
        return 1
    print(f"Ticket 38 core-union gate contract: {output}", flush=True)
    return 0
...
def _validate_source_dependencies(
    ...
    *,
    ...
    expected = build_ticket38_core_union_gate_contract()["source_dependencies"]
    if value != expected:
        raise ValueError("Ticket 38 gate source dependency contract drift")
    if source_texts is not None:
        if not isinstance(source_texts, Mapping):
            raise ValueError("Ticket 38 gate source texts must be an object")
        paths = {path for path, _sha256 in SOURCE_DEPENDENCIES}
        if any(path not in paths for path in source_texts):
            raise ValueError("Ticket 38 gate source text path is not declared")
    ...
    for path, expected_sha256 in SOURCE_DEPENDENCIES:
        ...
        if actual != expected_sha256:
            raise ValueError(f"Ticket 38 gate source dependency SHA-256 mismatch: {path}")


def _validate_candidate_artifacts(value: object) -> None:
    expected = build_ticket38_core_union_gate_contract()["candidate_artifacts"]
    if not isinstance(value, Mapping) or set(value) != set(expected):
        raise ValueError("Ticket 38 gate candidate artifact fields drift")
    if value.get("root") != CANDIDATE_ARTIFACT_ROOT:
        raise ValueError("Ticket 38 gate candidate artifact root drift")
    if value.get("hash_policy") != HASH_POLICY:
        raise ValueError("Ticket 38 gate candidate artifact hash policy drift")
    if value.get("seal_status") != "future_artifact_seal_required":
        raise ValueError("Ticket 38 gate candidate artifact seal status drift")
    roles = value.get("roles")
    if not isinstance(roles, list) or len(roles) != len(ARTIFACT_ROLES):
        raise ValueError("Ticket 38 gate candidate artifact roles drift")
    seen_roles: set[str] = set()
    seen_paths: set[str] = set()
    for item in roles:
        if not isinstance(item, Mapping) or set(item) != {"role", "path", "bytes", "sha256"}:
            raise ValueError("Ticket 38 gate candidate artifact role fields drift")
        role = item["role"]
        path = item["path"]
        if not isinstance(role, str) or role not in ARTIFACT_ROLES or role in seen_roles:
            raise ValueError("Ticket 38 gate candidate artifact roles must be unique and allowed")
        seen_roles.add(role)
        _validate_artifact_path(path)
        if path in seen_paths:
            raise ValueError("Ticket 38 gate candidate artifact paths must be unique")
        seen_paths.add(path)
        if path != f"{CANDIDATE_ARTIFACT_ROOT}/{ARTIFACT_FILENAMES[role]}":
            raise ValueError("Ticket 38 gate candidate artifact path does not match role")
        if item["bytes"] is not None or item["sha256"] is not None:
            raise ValueError("Ticket 38 gate candidate artifact hashes must remain pending")
    if tuple(item["role"] for item in roles) != ARTIFACT_ROLES:
        raise ValueError("Ticket 38 gate candidate artifact role order drift")


def _validate_artifact_path(path: object) -> None:
    if not isinstance(path, str) or not path:
        raise ValueError("Ticket 38 gate candidate artifact path must be relative")
    lowered = path.lower()
    if any(token in lowered for token in _FORBIDDEN_PATH_TOKENS):
        raise ValueError("Ticket 38 gate candidate artifact path contains forbidden evidence token")
    if (
        path.startswith(("/", "\\"))
        ...
        raise ValueError("Ticket 38 gate candidate artifact path must reject absolute or traversal paths")
    if not path.startswith(CANDIDATE_ARTIFACT_ROOT + "/"):
        raise ValueError("Ticket 38 gate candidate artifact path is outside allowed root")
...
def _read_normalized_file_bytes(path: Path, label: str) -> bytes:
    try:
        ...
        raise ValueError(f"cannot read Ticket 38 gate source dependency {label}: {error}") from error
